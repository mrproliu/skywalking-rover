...
const (
	...
	batchReadMinDuration = time.Second * 5
)
...
func (a *ProtocolAnalyzer) Start(ctx context.Context) {
	timeTicker := time.NewTicker(batchReadMinDuration)
	go func() {
		for {
			select {
			case <-timeTicker.C:
...
func (a *ProtocolAnalyzer) ReceiveSocketData(ctx Context, event *SocketDataUploadEvent) {
	...
	log.Debugf("receive connection: %s, dataid: %d, sequence: %d, finished: %d, message type: %s, direction: %s, size: %d, total size: %d",
		connectionID, event.DataID(), event.DataSequence(), event.Finished, event.MsgType.String(),
...
func (a *ProtocolAnalyzer) processConnectionEvents(connection *connectionInfo) {
	...
	for {
		...
		if !buffer.prepareForReading() {
			log.Infof("prepare finsihed: event size: %d", buffer.events.Len())
			return
		}
		...
		if finishReading {
			log.Infof("reading finsihed: event size: %d", buffer.events.Len())
			break
		}
	}
}
