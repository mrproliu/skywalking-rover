...
func (m *MessageOpt) EndTime() uint64 {
	return m.HeaderBuffer().FirstSocketBuffer().EndTime()
}
...
func (m *MessageOpt) checkChunkedBody(buf *protocol.Buffer, bodyReader *bufio.Reader) (*protocol.Buffer, protocol.ParseResult, error) {
	...
	for {
		...
		if b, r, err1 := m.checkBodyWithSize(buf, bodyReader, int(needBytes)); err1 != nil {
...
func (m *MessageOpt) checkBodyWithSize(buf *protocol.Buffer, reader *bufio.Reader, size int) (*protocol.Buffer, protocol.ParseResult, error) {
	reduceSize := size
